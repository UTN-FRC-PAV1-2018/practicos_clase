Partial Public Class frmBugs
    ...
    Private Sub btn_cerrar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_cerrar.Click
        If MessageBox.Show("Seguro que desea cerrar el Bug?", "Aviso", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.OK Then
            Dim id As Integer

            If row_selected.Index <> -1 Then
                id = Convert.ToInt32(row_selected.Cells("id_bug_col").Value)
                oBugService.cerrar(oBugService.consultarBugsPorId(id))
                btnConsultar_Click(sender, e)
            End If
        End If
    End Sub

    Private Sub btn_pasar_a_te_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_pasar_a_te.Click
        If MessageBox.Show("Seguro que desea pasar el Bug a Testing?", "Aviso", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.OK Then
            Dim id As Integer

            If row_selected.Index <> -1 Then
                id = Convert.ToInt32(row_selected.Cells("id_bug_col").Value)
                oBugService.pasarATesting(oBugService.consultarBugsPorId(id))
                btnConsultar_Click(sender, e)
            End If
        End If
    End Sub
